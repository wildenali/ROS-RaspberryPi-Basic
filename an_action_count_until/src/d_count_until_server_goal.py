...
class CountUntilServer:
  def __init__(self):
    self._as = actionlib.SimpleActionServer('/count_until_goal', CountUntilAsyncAction, execute_cb=self.on_goal, auto_start=False)
    self._as.start()

    self._counter = 0
    rospy.loginfo("Simple Action Goal Server has been started")
...
if __name__ == '__main__':
  rospy.init_node('count_until_server_goal')
